...
class WindowsPackagingTests(unittest.TestCase):
    ...
    def test_agent_api_launcher_uses_bundled_python_core(self) -> None:
        launcher = (WINDOWS / "worker/start_agent_api.cmd").read_text(encoding="utf-8")
        publish = (WINDOWS / "publish.ps1").read_text(encoding="utf-8")
        self.assertIn(r"%~dp0python-core\python.exe", launcher)
        self.assertIn(r"%~dp0worker\local_api.py", launcher)
        self.assertIn("'local_api.py'", publish)
        self.assertIn("'SPP Agent API.cmd'", publish)
    ...
    def test_windows_clone_page_matches_bundled_mac_defaults(self) -> None:
        ...
        self.assertIn("SizeInt32(1980, 1600)", source)
        ...
        self.assertIn('Background="#F7F7F8"', window_xaml)
        self.assertIn('ms-appx:///Assets/pixel_icon.png', source)
        self.assertIn('Width = 58', source)
        self.assertIn('Height = 58', source)
        self.assertIn('Text = "1.0 · Local AI"', source)
        self.assertIn('SetCloneBusy(true)', source)
        self.assertIn('DispatcherQueue.CreateTimer()', source)
        self.assertIn('Content = active ? "继续加入队列" : "生成声音"', source)
        self.assertIn('BuildCloneQueuePanel(output)', source)
        self.assertIn('ProcessCloneQueueAsync()', source)
        self.assertIn('ToggleInlinePlayback', source)
        self.assertIn('MediaSource.CreateFromStorageFile', source)
        self.assertIn('ActionButton("输出文件夹"', source)
        self.assertIn('titleBar.InactiveBackgroundColor = titleBackground', source)
        self.assertIn('HorizontalContentAlignment = HorizontalAlignment.Stretch', source)
        self.assertNotIn('MaxWidth = 1080', source)
        self.assertNotIn('运行状态会在右侧实时反馈', source)
        self.assertNotIn('点阵会随生成过程轻微呼吸', source)
        self.assertIn('PointerEntered', source)
        self.assertIn('ActionButton("复制诊断", CopyDiagnostics)', source)
        self.assertIn('ActionButton("打开模型目录", OpenModelDirectory)', source)
        self.assertIn('new ModelProgressVisual()', source)
        self.assertNotIn('new ProgressBar', source)
